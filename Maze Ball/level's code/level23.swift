...
class level23: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        backgroundNode = self.childNode(withName: "level23")
    ...
    @objc func delayedActionUp() {
        let moveUp = SKAction.moveTo(y: 200, duration: 1.5)
    ...
    @objc func delayedActionDown() {
        let moveDown = SKAction.moveTo(y: 145, duration: 1.5)
        spinnerNode.run(moveDown)
    }
    @objc func delayedActionDownWall() {
        let moveDownWall = SKAction.moveTo(y: 145, duration: 1.5)
    ...
    func stopGame() {
        let scene = SKScene(fileNamed: "EndGameLvl23")
