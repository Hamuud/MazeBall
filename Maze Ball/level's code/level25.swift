...
class level25: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        backgroundNode = self.childNode(withName: "level25")
    ...
    @objc func delayedActionUp() {
        let moveUp = SKAction.moveTo(x: -182, duration: 1.5)
         spinnerNode.run(moveUp)
        }
    
    @objc func delayedActionUpWall() {
        let moveUpWall = SKAction.moveTo(x: -182, duration: 1.5)
         wallNode.run(moveUpWall)
        }
    
    @objc func delayedActionDown() {
        let moveDown = SKAction.moveTo(x: -115, duration: 1.5)
        spinnerNode.run(moveDown)
    }
    @objc func delayedActionDownWall() {
        let moveDownWall = SKAction.moveTo(x: -115, duration: 1.5)
    ...
    func stopGame() {
        let scene = SKScene(fileNamed: "EndGameLvl25")
