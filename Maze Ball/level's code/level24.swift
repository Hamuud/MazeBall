...
class level24: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
       ...
       /* spinnerNode = self.childNode(withName: "Spinner2")
        ...
        spinnerNode.run(.rotate(toAngle: -100 * 3.14, duration: 300))*/
    ...
    @objc func delayedActionUp() {
        let moveUp = SKAction.moveTo(y: -150, duration: 1.5)
         spinnerNode.run(moveUp)
        }
    
    @objc func delayedActionUpWall() {
        let moveUpWall = SKAction.moveTo(y: 205, duration: 1.5)
         wallNode.run(moveUpWall)
        }
    
    @objc func delayedActionDown() {
        let moveDown = SKAction.moveTo(y: -200, duration: 1.5)
        spinnerNode.run(moveDown)
    }
    @objc func delayedActionDownWall() {
        let moveDownWall = SKAction.moveTo(y: 155, duration: 1.5)
    ...
    func stopGame() {
        let scene = SKScene(fileNamed: "EndGameLvl24")
